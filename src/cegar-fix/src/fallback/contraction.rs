...
impl Degree2Contractor {
    ...
    /// Contracts degree-2 chains in the graph without protecting any vertices.
    pub fn contract(g: &Graph) -> (Graph, Degree2Contractor) {
        Self::contract_with_protected(g, &HashSet::new())
    }

    /// Contracts degree-2 chains in the graph, skipping any vertex in `protected`.
    /// Protected vertices are never contracted and instead act as chain endpoints.
    pub fn contract_with_protected(
        g: &Graph,
        protected: &HashSet<i32>,
    ) -> (Graph, Degree2Contractor) {
        ...
        // Identify degree-2 vertices, skipping protected vertices
        let deg2_vertices: HashSet<i32> = adj
            .iter()
            .filter(|(&u, neighbors)| neighbors.len() == 2 && !protected.contains(&u))
            .map(|(&u, _)| u)
            .collect();
        ...
        for &v in &deg2_vertices {
            ...
            let end_u = curr; // non-degree-2 or protected vertex
            ...
            let end_w = curr; // non-degree-2 or protected vertex
...
mod tests {
    ...
    #[test]
    fn test_contract_with_protected() {
        // Graph: 1 - 2 - 3, 1-4, 3-4, 1-5, 3-5, 4-5
        // Vertex 2 has degree 2. If vertex 2 is protected, it should NOT be contracted.
        let edges = vec![
            (1, 2), (2, 3),
            (1, 4), (3, 4),
            (1, 5), (3, 5),
        ...
        let (cg, contractor) = Degree2Contractor::contract_with_protected(&g, &protected);
        assert!(!contractor.is_infeasible);
        assert_eq!(contractor.original_vertices_count, 5);
        assert_eq!(contractor.contracted_vertices_count, 5);
        assert_eq!(contractor.chain_map.len(), 0);
        assert_eq!(contractor.forced_edges.len(), 0);
        assert!(cg.adjacency_list.get(&2).is_some());
        assert_eq!(cg.adjacency_list.get(&2).unwrap().len(), 2);
    }
