...
use cegar_fix::macro_lfsr_encoder::MacroLfsrEncoder;
use rustsat::solvers::{Solve, SolverResult};
use rustsat::types::{Lit, Var};
use rustsat_cadical::CaDiCaL;

#[test]
fn test_macro_lfsr_encoder_basic() {
    ...
    let encoder = MacroLfsrEncoder::encode(
        &mut solver,
        &mut next_var_id,
        &decomp,
        &var_hh,
    ...
    assert_eq!(encoder.specs.len(), 2); // 2 specs for N <= 100
    assert_eq!(encoder.root_hub, 1);
    assert_eq!(encoder.bit_vars.len(), 5 * 2); // 5 hubs * 2 specs
