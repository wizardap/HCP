...
def solve_graph(graph_file):
    ...
    print(f"Contracted: {len(G)} -> {len(rem)} vertices in {time.time()-t0:.2f}s", flush=True)
    ...
    print(f"Formulation built in {time.time()-t0:.2f}s. Starting CEGAR with phase saving...", flush=True)
    winner = None
    for it in range(1, 301):
        ...
        # Phase saving: retain polarities of all edge variables from the model
        ...
        if len(cycles) == 1:
            print(f"[✓] Exact SAT cycle found at iter {it} in {time.time()-t0:.2f}s!", flush=True)
            winner = cycles[0]
            break
            
        # Pure 2-opt cycle absorption
        merged = list(cycles)
        merged_any = True
        ...
        if len(merged) == 1:
            print(f"[✓] Absorbed 2-opt cycle found at iter {it} in {time.time()-t0:.2f}s!", flush=True)
            winner = merged[0]
            break
            
        # Non-intrusive closing operator for len(merged) == 2
        if len(merged) == 2 and (len(merged[0]) <= 40 or len(merged[1]) <= 40):
            c_big = merged[0] if len(merged[0]) >= len(merged[1]) else merged[1]
            c_small = merged[1] if len(merged[0]) >= len(merged[1]) else merged[0]
            resp = try_patch_merge(c_big, c_small, adj, contracted_edges, max_window=10)
            if resp:
                print(f"[✓] Closed via patch merge at iter {it} in {time.time()-t0:.2f}s!", flush=True)
                winner = resp
                break
            res3 = try_merge_3opt(c_big, c_small, adj, contracted_edges)
            if res3:
                print(f"[✓] Closed via 3-opt at iter {it} in {time.time()-t0:.2f}s!", flush=True)
                winner = res3
                break
        ...
        # Add cocycle cuts and negative cuts on RAW cycles (never on merged!)
        for cyc in cycles:
    ...
    if winner:
        ...
        print(f"[✓] SUCCESS! Solved {graph_file} in {time.time()-t0:.2f}s total!", flush=True)
        return full
    return None

if __name__ == '__main__':
    import sys
    gfile = sys.argv[1] if len(sys.argv) > 1 else 'FHCPCS-col/graph788.col'
    solve_graph(gfile)
