...
def _expand_and_save_cycle(cyc: List[int], edge_chains: Dict[Tuple[int, int], List[int]], comp0_nodes: Set[int], cache_path: str, t0: float, solver) -> List[int]:
    ...
    print(f"[Comp 0] Expanded cycle length: {len(expanded)} (expected {len(comp0_nodes)}).")
...
def solve_comp0(G: Dict[int, Set[int]], comp0_nodes: Set[int], p1_ext: int, p2_ext: int, cache_path: str = None) -> List[int]:
    if cache_path and os.path.exists(cache_path):
        with open(cache_path, "r") as f:
            cyc = json.load(f)
        if len(cyc) == len(comp0_nodes) and len(set(cyc)) == len(comp0_nodes):
    ...
    # Force virtual edge and contracted macro-edges
    ...
    while True:
        ...
        if it % 10 == 0 or len(merged_cycles) <= 5:
            cyc_lens = sorted([len(c) for c in cycles], reverse=True)
            abs_lens = sorted([len(c) for c in merged_cycles], reverse=True)
            print(f"[Comp 0] Iter {it} ({time.time()-t_start_it:.2f}s): {len(cycles)} raw -> {len(merged_cycles)} absorbed (max {abs_lens[0]}, min {abs_lens[-1]})")
...
if __name__ == "__main__":
    ...
    solve_comp0(G, comp0_nodes, p1_ext, p2_ext)
