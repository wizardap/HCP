...
struct ThreadTerminator<'a>(&'a AtomicBool);
impl<'a> Terminate for ThreadTerminator<'a> {
    fn terminate(&mut self) -> ControlSignal {
...
fn parallel_solve_cnf(cnf: &Cnf, num_workers: usize, round: usize) -> Option<Vec<Lit>> {
    let num_workers = num_workers.max(1);
    let cancelled = AtomicBool::new(false);
    let (tx, rx) = mpsc::channel::<Option<Vec<Lit>>>();
    ...
    thread::scope(|s| {
        for worker_id in 0..num_workers {
            ...
            s.spawn(move || {
                ...
                match worker_id {
                    0 => {} // Worker 0: Default deterministic
                    1 => {
                ...
                solver.attach_terminator(ThreadTerminator(cancelled_ref));

                let res = solver.solve();
                if matches!(res, Ok(SolverResult::Sat)) {
                    if !cancelled_ref.swap(true, Ordering::SeqCst) {
                        let sol = solver.full_solution().unwrap();
                        let _ = tx_worker.send(Some(sol.into_iter().collect()));
                    }
                } else if matches!(res, Ok(SolverResult::Unsat)) {
                    let _ = tx_worker.send(None);
                }
            });
        }
        drop(tx);
...
fn main() {
    ...
    // PRE-INJECT STATIC 4-CYCLES WITH DUAL BOUNDARY CUTS AT ROUND 0
    let mut static_4_cycles = Vec::new();
    for u in 0..n_dir {
        for &v in &dir_adj[u] {
            for &w in &dir_adj[v] {
                if w != u {
                    for &x in &dir_adj[w] {
                        if x != v && dir_adj[x].contains(&u) {
                            if u < v && u < w && u < x {
                                static_4_cycles.push(vec![u, v, w, x]);
                            }
                        }
                    }
                }
    ...
    println!("Pre-injecting {} static 4-cycles with Dual Boundary Cuts at Round 0...", static_4_cycles.len());
    for c in &static_4_cycles {
        let lits = vec![
            !arc_lit_map[&(c[0], c[1])],
            !arc_lit_map[&(c[1], c[2])],
            !arc_lit_map[&(c[2], c[3])],
    ...
    println!("Starting 3-Core Parallel Portfolio Solving on {} (N_dir={}, Arcs={}, Clauses={})...",
        graph_path, n_dir, arc_set.len(), cnf.len());

    for round in 1..=1000 {
        let t_round = Instant::now();
        let sol_opt = parallel_solve_cnf(&cnf, 3, round);
        if sol_opt.is_none() {
            println!("Parallel Portfolio returned UNSAT / No solution");
            break;
        }
        ...
        for c in &cycles {
            if c.len() < n_dir {
                ...
                if c.len() <= 64 {
                    ...
                    if !in_lits.is_empty() {
                        cuts.add_clause(Clause::from_iter(in_lits));
                    }
                }
            }
        }
