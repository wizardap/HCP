...
fn main() {
    ...
    println!("Starting Directed Dual-Cut Solving on {} (N_dir={}, Arcs={})...", graph_path, n_dir, arc_set.len());
    ...
    for round in 1..=1000 {
        ...
        for c in &cycles {
            if c.len() < n_dir {
                ...
                // DUAL BOUNDARY CUTS: Both outgoing AND incoming cuts for all subcycles <= 64
                if c.len() <= 64 {
                    let c_set: HashSet<usize> = c.iter().copied().collect();
                    // Outgoing boundary cut
