...
# 2-cycle mutex for pairs that are NOT existing 2-cycles
for (u, v), var in arc_to_var.items():
    if u < v and (v, u) in arc_to_var:
        # If this is not an existing 2-cycle, ban it
        if not ((u, v) in init_arcs and (v, u) in init_arcs):
...
print(f"Testing route generation on all 20 components with v2 CEGAR...")
routes_found = {}
...
for c_id in range(len(comps)):
    ...
    # Ban current arcs of this component: for each subcycle, at least one arc must change
    for sid in comp:
        solver.add_clause([-arc_to_var[arc] for arc in sub_arcs[sid - 1]])
        
    # If this component is a 2-cycle, ban both arcs simultaneously
    for sid in comp:
        sc_a = sub_arcs[sid - 1]
        if len(sc_a) == 2:
            solver.add_clause([-arc_to_var[sc_a[0]], -arc_to_var[sc_a[1]]])
    ...
    if found_route is None:
        print(f"Comp {c_id:2d} ({len(comp_blocks):2d} blocks): FAILED (UNSAT or CEGAR limit {cegar_it})")
    else:
