# CNF Subsumer & Polarity-Guided Backbone Engine Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Implement `CnfSubsumer` to deduplicate and eliminate subsumed conflict cut clauses across deep CEGAR rounds, preventing CaDiCaL clause explosion and keeping SAT solve times low.

**Architecture:** Create `src/cegar-fix/src/cnf_subsumer.rs`, wire into `src/cegar-fix/src/hcp_solver.rs`.

**Tech Stack:** Rust (2021 edition), CaDiCaL SAT solver (`rustsat`, `rustsat_cadical`).

## Global Constraints
- Target directory: `/home/ubuntu/HCP/src/cegar-fix`
- Core Reservation: Core 3 is strictly reserved for the user. Run all tasks and commands with `taskset -c 0,1,2 nice -n 19`.
- Zero Tour Injection: Absolutely NO importing, reading, or referencing `.hcp.tou` files during solving.
- Benchmark Time Limit: $T_{\max} = 1800\text{s}$.
- Empirical Rigor: No overpromising. Maintain strict verification.

---

### Task 1: `CnfSubsumer` Engine

**Files:**
- Create: `src/cegar-fix/src/cnf_subsumer.rs`
- Modify: `src/cegar-fix/src/lib.rs`, `src/cegar-fix/src/main.rs` (export `pub mod cnf_subsumer;`)
- Test: `src/cegar-fix/tests/test_cnf_subsumer.rs`

**Interfaces:**
```rust
use rustsat::instances::Cnf;
use rustsat::types::{Clause, Lit};
use std::collections::HashSet;

pub struct CnfSubsumer;

impl CnfSubsumer {
    pub fn prune_and_subsume_cuts(cnfs: &[Cnf]) -> Cnf;
}
```

- [ ] **Step 1: Write unit tests** in `src/cegar-fix/tests/test_cnf_subsumer.rs`
- [ ] **Step 2: Run test to verify it fails** (`taskset -c 0,1,2 nice -n 19 cargo test --test test_cnf_subsumer`)
- [ ] **Step 3: Implement `CnfSubsumer` in `src/cegar-fix/src/cnf_subsumer.rs`**
- [ ] **Step 4: Run test to verify it passes** (`taskset -c 0,1,2 nice -n 19 cargo test --test test_cnf_subsumer`)
- [ ] **Step 5: Commit changes**

---

### Task 2: Wire `CnfSubsumer` into `hcp_solver.rs`

**Files:**
- Modify: `src/cegar-fix/src/hcp_solver.rs`
- Test: `src/cegar-fix/tests/test_staged_solver.rs`

- [ ] **Step 1: Wire `CnfSubsumer::prune_and_subsume_cuts` into `SolverReseeder` and CEGAR loop**
- [ ] **Step 2: Add integration test in `src/cegar-fix/tests/test_staged_solver.rs`**
- [ ] **Step 3: Run full workspace test suite** (`taskset -c 0,1,2 nice -n 19 cargo test`)
- [ ] **Step 4: Commit changes**

---

### Task 3: Benchmark Verification on `graph479.col` & `graph668.col`

**Files:**
- Verify: `FHCPCS-col/graph479.col` and `FHCPCS-col/graph668.col`

- [ ] **Step 1: Build release binary** (`taskset -c 0,1,2 nice -n 19 cargo build --release`)
- [ ] **Step 2: Run full workspace test suite** (`taskset -c 0,1,2 nice -n 19 cargo test`)
- [ ] **Step 3: Run benchmark on `graph479.col` and `graph668.col`**
- [ ] **Step 4: Document results and commit**
